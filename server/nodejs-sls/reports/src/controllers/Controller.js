    ...
    try {
      ...
      if (validationSchema) {
        ...
        if (!validationObj) {
          ...
          return Controller.sendError(response, errRespObj);
        }
      }
      if (request.identity) {
      ...
      const serviceResponse = await serviceOperation(consolidatedParams);console.log('response-->', JSON.stringify(serviceResponse));
